...
PRIV_CONFIG_METDATA_FILE = '/etc/bluedata/.priv_configmeta.json' # not present in k8s version
PLATFORM_INFO_METADATA_FILE = '/etc/bluedata/.platform.json' # not present in k8s version
...
def _resolve_indirections(keyTokenList, jsonData):
    ...
    if isinstance(data, dict):
        ...
        raise KeyTokensRemainingException()
         ...
         isinstance(data, str) or isinstance(data, bool) or isinstance(data, int):
        return data
    else:
...
class BDVLIB_ConfigMetadata(object):
    ...
    def get(self, key, delim='.'):
        ...
        if (key == "namespaces") or (key == ''):
            return ','.join(["version", "node", "cluster", "distro", "services",
                             "tenant", "auth", "platform", "attachments"])
    ...
    def getLocalGroupHosts(self):
        ...
        """
        LocalNodeGrpId = self.getWithTokens(["node", "nodegroup_id"])
    ...
    def getClusterHostsFQDN(self):
        ...
        """
        NodeGroups = self.getWithTokens(["nodegroups"])
    ...
    def getNodeGroupFQDN(self, nodeGroupId):
        matchedKeyTokenLists = _search_token_recursive(["nodegroups",
                                                        nodeGroupId],
                                                       "fqdns", self.jsonData)
    ...
    def getNumNodegroups(self):
        ...
        """
        return len(self.getWithTokens(["nodegroups"]))
    ...
    def getTenantInfoKey(self, key):
        ...
        """
        return self.getWithTokens(["cluster",
                                   "tenant_info",
                                   key])
    def getTenantInfo(self):
        ...
        """
        TenantInfoKeys = self.getWithTokens(["cluster", "tenant_info"])
        PropsList = []
